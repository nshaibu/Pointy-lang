...
class ControlFlowEvent(EventBase):
    ...
    def process(self, *args, **kwargs):
        # Implementation of mini-orchestrator
        pass
```
...
- Single values: Wrapped as a single-item list
- None: Treated as empty list `[]`
- Dictionaries: Treated as a single item (not flattened)
...
**Behavior**:
- Raise `ExecutorIncompatibilityError` at initialisation
...
Namespaces provide a mechanism to uniquely identify and differentiate events that may share the same name but originate from different sources. The namespace system enables Pointy Language to load and orchestrate events from multiple registries and sources without naming conflicts.

### 11.1 Namespace Syntax

```
namespaced_event ::= (namespace '::')? identifier
namespace        ::= identifier
```

**Examples:**
- `pypi::LoadData`
- `github::ProcessPayment`
- `local::ValidateInput`
- `myorg::TransformData`

### 11.2 Namespace Operator

**Operator:** `::`

**Syntax:** `namespace::event_name`

**Semantics:**
- The `::` operator separates the namespace from the event name
- Namespace resolution occurs before event execution
- Namespace is case-sensitive

### 11.3 Supported Event Sources

The runtime supports loading events from the following sources:

| Source Type | Namespace Identifier | Description |
|-------------|---------------------|-------------|
| Local | `local` | Events defined in the local project/workspace |
| PyPI | `pypi` | Events installed from Python Package Index |
| GitHub | `github` | Events loaded from GitHub repositories |
| Custom Registry | User-defined | Organization-specific or custom registries |

### 11.4 Namespace Resolution Rules

1. **Explicit Namespace**: When a namespace is specified, the runtime resolves the event exclusively from that source
   ```pointy
   pypi::LoadData -> github::ProcessData
   ```

2. **Implicit Namespace (Default)**: When no namespace is specified, the runtime assumes the `local` namespace (current project)
   
   ```pointy
   LoadData -> ProcessData  # Resolved using local namespace
   ```

3. **Uniqueness Requirement**: Within a single namespace, event names must be unique


### 11.5 Namespace in Complex Expressions

Namespaces can be used in all event expressions:

#### Sequential Flow
```pointy
local::ValidateOrder -> pypi::ProcessPayment -> github::SendConfirmation
```
...
#### Result Piping
```pointy
local::ExtractData |-> pypi::TransformData |-> github::LoadData
```

#### Conditional Branching
```pointy
pypi::ValidatePayment (
    0 -> local::LogError -> github::NotifyAdmin,
    1 -> pypi::ProcessOrder
)
```
...
```pointy
github::ConnectToAPI * 3 -> pypi::ProcessResponse
```

### 11.6 Best Practices

1. **Explicit Namespaces**: Use explicit namespaces for production workflows to ensure deterministic event resolution
2. **Namespace Aliases**: Consider using short, meaningful namespace identifiers
3. **Version Pinning**: When loading from external sources, pin to specific versions
4. **Local Development**: Use the `local` namespace during development, explicit namespaces in production
5. **Documentation**: Document namespace sources and their purposes in workflow comments

#### Lexical Token Updates
```
namespace_separator ::= '::'
```

#### Updated Identifier Grammar
```
...
Directives are special declarations that configure the execution environment and parsing behavior of Pointy Language workflows. They provide compile-time and runtime configuration options that affect how the parser interprets and executes pipeline definitions.

### 12.1 Directive Syntax

```
directive       ::= '@' directive_name ':' directive_value
directive_name  ::= identifier
directive_value ::= identifier | number | string_literal
string_literal  ::= '"' [^"]* '"'
number          ::= digit+
```

**General Form:** `@directive_name:value`
...
### 12.2 Directive Placement

Directives must be placed at the beginning of a Pointy Language script, before any event expressions:

```pointy
@mode:"DAG"
@recursive-depth:3000

LoadData -> ProcessData -> SaveResults
```

**Rules:**
- Directives must appear before the first event expression
- Multiple directives can be specified, one per line
- Directives are processed in the order they appear
- Blank lines and comments are allowed between directives

### 12.3 Standard Directives

#### 12.3.1 Mode Directive

**Syntax:** `@mode:"DAG"` | `@mode:"CFG"`

**Purpose:** Specifies the parsing mode for workflow interpretation

**Values:**

| Value | Description | Characteristics |
|-------|-------------|-----------------|
| `DAG` | Directed Acyclic Graph | - No cycles allowed<br>- Strict topological ordering<br>- Optimized for pipeline-style workflows<br>- Better performance for linear flows |
| `CFG` | Control Flow Graph | - Cycles permitted<br>- Supports loops and recursive patterns<br>- More flexible control structures<br>- Required for iterative workflows |
...
```pointy
# DAG mode - strict acyclic workflow
@mode:"DAG"
...
```pointy
# CFG mode - allows cyclic patterns
@mode:"CFG"
...
**Validation:**
- In DAG mode, the parser will reject workflows with cycles
- In CFG mode, cycles are allowed but infinite loops must be prevented by event logic
- Mode must be specified before any event expressions

**Semantic Implications:**
- **DAG Mode:**
  - Enables aggressive optimization through topological sorting
  - Guarantees workflow termination (no infinite loops)
  - Ideal for ETL pipelines and data processing workflows
  - Allows parallel execution analysis at parse time

- **CFG Mode:**
  - Supports iterative and recursive workflow patterns
  - Enables loop constructs through conditional back-edges
  - Requires runtime cycle detection and loop management
  - Suitable for state machines and iterative processes

#### 12.3.2 Recursive Depth Directive

**Syntax:** `@recursive-depth:number`

**Purpose:** Sets the maximum recursion depth for the parser and execution engine

**Value:** Positive integer representing maximum recursion depth

**Default:** Implementation-defined (typically 1000)

**Range:** 1 to implementation-defined maximum (system-dependent)
...
DeepNestedWorkflow -> ProcessLayer1 -> ProcessLayer2 -> ProcessLayerN
```

**Use Cases:**
- Deep workflow nesting
- Complex conditional branching with many levels
- Recursive event patterns in CFG mode
- Large-scale pipeline definitions with extensive branching

**Behavior:**
- Affects both parser recursion during workflow analysis
- Affects execution engine recursion during event processing
- Parser will raise an error if depth is exceeded during parsing
- Runtime will raise an error if depth is exceeded during execution

**Warnings:**
- Very high values may impact performance and memory usage
- Stack overflow risks with excessive recursion
- Should be set based on actual workflow requirements
- Different platforms may have different maximum limits

**Performance Considerations:**
- Each recursion level consumes stack space
- Higher values increase memory footprint
- May affect garbage collection behavior
- Test workflows with target recursion depth before production deployment

### 12.4 Directive Scoping

**Global Scope:**
All directives have global scope and affect the entire pipeline definition. There is no mechanism for local directive overrides within a single script.

**No Local Override:**
Directives cannot be changed or overridden within the script. Only one value per directive is active for the entire workflow.

**Conflict Resolution:**
If the same directive is specified multiple times:
- **Last-wins:** The last specified value takes precedence
- **Warning:** Parser should issue a warning about duplicate directives
- **Best Practice:** Specify each directive only once

### 12.5 Directive Validation

The parser must validate directives according to these rules:

1. **Syntax Validation:**
   - Directive must follow `@name:value` format
   - Directive name must be a valid identifier
   - Value must be appropriate for the directive type

2. **Value Validation:**
   - Mode values must be either "DAG" or "CFG"
   - Recursive depth must be a positive integer
   - Values must be within acceptable ranges

3. **Placement Validation:**
   - All directives must appear before any event expressions
   - Directives after event expressions result in parse error

4. **Compatibility Validation:**
   - Directive combinations must be logically consistent
   - Implementation may enforce additional compatibility rules

### 12.6 Lexical Token Updates
...
#### Updated Grammar
```
pipeline       ::= directive_list? expression
directive_list ::= directive+
directive      ::= '@' identifier ':' (identifier | number | string_literal)
```

### 12.7 Interaction with Other Language Features

#### 12.7.1 Directives and Namespaces

Directives apply to all events regardless of namespace:

```pointy
@mode:"DAG"
@recursive-depth:2000

local::LoadData -> pypi::ProcessData -> github::SaveData
```

#### 12.7.2 Directives and Error Handling

In DAG mode with cycles detected:
...
A -> B (
    0 -> C,
    1 -> A  # ERROR: Cycle detected in DAG mode
)
```

In CFG mode, the same pattern is allowed:
...
A -> B (
    0 -> C,
    1 -> A  # OK: Cycle allowed in CFG mode
)
```

#### 12.7.3 Directives and Operators

All operators work identically in both DAG and CFG modes, except:
- DAG mode prohibits cycles through any operator combination
- CFG mode allows cycles but requires careful event design to prevent infinite loops
...
LoadCustomers -> TransformData -> SaveToDatabase
```

#### Complex CFG Workflow
```pointy
@mode:"CFG"
@recursive-depth:5000
...
#### Iterative Processing (CFG Mode)
```pointy
@mode:"CFG"
@recursive-depth:2000

InitializeCounter -> ProcessItem (
    0 -> Finalize,  # Counter reached limit
    1 -> TransformItem -> IncrementCounter -> ProcessItem
)
```

#### Multi-namespace with Directives
```pointy
@mode:"DAG"
@recursive-depth:1500

local::FetchOrders 
    -> pypi::ValidateOrders 
    -> github::EnrichWithCustomerData 
    || local::CalculateTotals 
    |-> pypi::GenerateInvoice
```

### 14.9 Future Extensions

The directive system is designed to be extensible. Future versions of Pointy Language may introduce additional directives while maintaining backward compatibility with existing scripts.

### 14.9.1 Backward Compatibility

Scripts without directives continue to work with default values:
- Default mode: `CFG`

This ensures existing Pointy Language workflows remain functional when the directive system is used.

---

**Note:** Currently, Pointy Language supports only the `@mode` and `@recursive-depth` directives. Additional directives may be added in future versions of the language specification.
