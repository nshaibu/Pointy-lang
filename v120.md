...
**Scenario**: A Template Event itself contains a Meta Event that fails.

**Behavior**:
- Inner Meta Event failure propagates to Template Event
- Template Event returns descriptor 0
- Outer Meta Event sees Template Event failure
- Error information is nested in structure

**Example**:
```pointy
# Outer MAP calls Template Event that internally uses MAP
@groups = [
    {"items": [1, 2, 3]},
    {"items": [4, 5, 6]},
    {"items": [7, 8, 9]}
...
LoadGroups |-> MAP<ProcessGroup>[concurrent=true] (
    0 -> HandleNestedFailure,
    1 -> AggregateResults
)

# Where ProcessGroup internally does:
...
```python
{
    "meta_event_id": "meta_outer_001",
    "mode": "MAP",
    "failed_tasks": 1,
    "errors": [
        {
            "task_id": "meta_outer_001_ProcessGroup_1",
            "error_type": "NestedMetaEventFailure",
            "nested_error": {
                "meta_event_id": "meta_inner_001",
                "mode": "MAP",
                "failed_tasks": 2,
                "errors": [...]  # Inner failures
            }
        }
    ]
}
...
**Behavior**:
- Detected at pipeline initialization
- Raise `CircularDependencyError`
- Pipeline fails before execution

**Example**:
```pointy
# EventA internally calls MAP<EventB>
# EventB internally calls MAP<EventA>
# This creates a circular dependency

LoadData |-> MAP<EventA>  # Raises CircularDependencyError
```

**Detection**: Dependency graph analysis occurs during pipeline initialization phase.
...
```pointy
# Process groups of items, where each group requires mapping
@grouped_data = [
    {"group": "A", "items": [1, 2, 3]},
    {"group": "B", "items": [4, 5, 6]}
]
...
LoadGroupedData |->
MAP<ProcessGroup>[concurrent=true] |->  # Each ProcessGroup internally uses MAP
